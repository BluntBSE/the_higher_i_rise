...
@export var button_class:String
@export var execute:String

# Called when the node enters the scene tree for the first time.
func _ready():
	get_node("button_text").text = text
	state_machine.Add("hover_state", MMHoverButtonState.new(self, "init hover args"))
	state_machine.Add("normal", MMNormalButtonState.new(self, "init normal MM button args"))
